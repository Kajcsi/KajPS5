...
// src/graphics/shader/recompiler/cfg/ShaderCFG.cpp at
...
namespace kajps5::gpu::shader::recompiler::CFG {
namespace {
...
bool SplitSharedMergeBlocks(Graph& graph, std::string* error) {
	...
	SetFailure(graph, FailureKind::StructuredControlFlow, graph.entry_block,
	           std::format("CFG shared merge splitting exceeded budget: original_blocks={} "
	...
	for (const auto& inst: program.instructions) {
		...
		if (inst.opcode == Opcode::Unsupported) {
			SetFailure(graph, FailureKind::UnsupportedInstruction, UINT32_MAX,
			           std::format("unsupported decoded instruction in CFG at pc 0x{:08x}: {}",
		...
		if (IsBranch(inst.opcode)) {
			if (!IsValidTarget(inst.branch_target, instruction_pcs, first_pc, end_pc)) {
				SetFailure(graph, FailureKind::InvalidBranchTarget, UINT32_MAX,
				           std::format("branch at pc 0x{:08x} targets invalid pc 0x{:08x}", inst.pc,
				                       inst.branch_target),
				           error);
				return false;
			}
			...
			if (!ResolveSetpcTargets(program, i, target_info)) {
				SetFailure(graph, FailureKind::InvalidBranchTarget, UINT32_MAX,
				           std::format("unsupported dynamic S_SETPC_B64 at pc 0x{:08x}", inst.pc),
				           error);
				return false;
			}
			...
			for (const auto target: target_pcs) {
				if (!IsValidTarget(target, instruction_pcs, first_pc, end_pc)) {
					SetFailure(graph, FailureKind::InvalidBranchTarget, UINT32_MAX,
					           std::format("S_SETPC_B64 at pc 0x{:08x} targets invalid pc 0x{:08x}",
					                       inst.pc, target),
					           error);
					return false;
				}
	...
	for (uint32_t i = 0; i < sorted_labels.size(); i++) {
		...
		if (start != end_pc && !instruction_pcs.contains(start)) {
			SetFailure(graph, FailureKind::InvalidLabel, UINT32_MAX,
			           std::format("CFG label does not start on an instruction: 0x{:08x}", start),
			           error);
			return false;
		}
		...
		if (last.opcode == Opcode::SEndpgm) {
			...
			if (fallthrough == pc_to_block.end()) {
				SetFailure(graph, FailureKind::MissingFallthrough, block.id,
				           std::format("conditional branch at pc 0x{:08x} has no fallthrough block",
				                       last.pc),
				           error);
				return false;
			}
	...
	for (const auto& loop: graph.natural_loops) {
		...
		if (header == nullptr || loop.merge == UINT32_MAX || loop.continue_block == UINT32_MAX) {
			SetFailure(
			    ...
			    std::format("loop at block {} has no structured merge/continue", loop.header),
			    error);
			return false;
		}
	...
	for (auto& block: graph.blocks) {
		...
		if (merge == UINT32_MAX) {
			SetFailure(graph, FailureKind::StructuredControlFlow, block.id,
			           std::format("conditional block {} has no structured merge", block.id),
			           error);
			return false;
		}
...
std::string FormatBlockDiagnostic(const Graph& graph, uint32_t block_id, const char* stage,
	...
	if (block == nullptr) {
		return std::format("stage={} block={} reason={}", stage_text, block_id, reason.c_str());
	}
	return std::format("stage={} block={} pc=0x{:08x}..0x{:08x} preds={} succs={} reason={}",
...
std::string GraphToString(const Graph& graph) {
	std::string text;
	text +=
	    std::format("entry_block={} irreducible={} unsupported={} failure={} failure_block={}\n",
	...
	for (const auto& edge: graph.back_edges) {
		text += std::format("backedge {} -> {} natural={}\n", edge.from, edge.to,
		                    edge.natural ? 1u : 0u);
	}
	for (const auto& loop: graph.natural_loops) {
		text += std::format("loop header={} latch={} merge={} continue={} body=[{}] exits=[{}]\n",
	...
	for (const auto& component: graph.components) {
		text += std::format("scc blocks=[{}] entries=[{}] irreducible={}\n",
