...
// src/graphics/shader/recompiler/ShaderRecompiler.cpp at
...
#include "gpu/shader/recompiler/ir/ReadLaneElimination.h"
...
// Each embedded table is a two-SGPR pair after the fixed PS5 ABI shift.
constexpr int kEmbeddedFetchSgprShift       = 8;
constexpr int kEmbeddedFetchSgprCount       = 108;
constexpr int kEmbeddedFetchRegisterMinimum = -kEmbeddedFetchSgprShift;
constexpr int kEmbeddedFetchRegisterMaximum =
    kEmbeddedFetchSgprCount - kEmbeddedFetchSgprShift - 2;

bool CanMapEmbeddedFetchRegisterPair(int register_id) {
	return register_id >= kEmbeddedFetchRegisterMinimum &&
	       register_id <= kEmbeddedFetchRegisterMaximum;
}

bool ValidateStageMetadata(const CompileOptions& options, std::string* error) {
	if (options.stage == ShaderType::Vertex && options.vertex_input_info != nullptr) {
		const auto* vertex_info  = options.vertex_input_info;
		const auto  resources_num = vertex_info->resources_num;
		if (resources_num < 0 || resources_num > ShaderVertexInputInfo::RES_MAX) {
			if (error != nullptr) {
				*error = "vertex shader metadata resources_num must be between 0 and 32";
			}
			return false;
		}
		if (vertex_info->fetch_embedded &&
		    !CanMapEmbeddedFetchRegisterPair(vertex_info->fetch_attrib_reg)) {
			if (error != nullptr) {
				*error = "embedded vertex fetch attribute register is out of range";
			}
			return false;
		}
		if (vertex_info->fetch_embedded &&
		    !CanMapEmbeddedFetchRegisterPair(vertex_info->fetch_buffer_reg)) {
			if (error != nullptr) {
				*error = "embedded vertex fetch buffer register is out of range";
			}
			return false;
		}
	}
	if (options.stage == ShaderType::Pixel && options.pixel_input_info != nullptr &&
	    options.pixel_input_info->input_num >
	        std::size(options.pixel_input_info->interpolator_settings)) {
		if (error != nullptr) {
			*error = "pixel shader metadata input_num exceeds 32";
		}
		return false;
	}
	if (options.stage == ShaderType::Compute && options.compute_input_info != nullptr) {
		const auto* compute_info = options.compute_input_info;
		if (compute_info->thread_ids_num < 0 || compute_info->thread_ids_num > 3) {
			if (error != nullptr) {
				*error = "compute shader metadata thread_ids_num must be between 0 and 3";
			}
			return false;
		}
		if (compute_info->wave_size != 0 && compute_info->wave_size != options.wave_size) {
			if (error != nullptr) {
				*error = "compute shader metadata wave_size must match shader wave size";
			}
			return false;
		}
	}
...
bool NeedsDispatcherForStructuredLoopHeader(const IR::Program& ir, std::string* reason) {
	for (const auto& block: ir.blocks) {
		...
		for (const auto& inst: block.instructions) {
			...
			if (reason != nullptr) {
				*reason = "loop header " + std::to_string(block.id) +
			          " contains an instruction whose SPIR-V lowering emits internal control flow: " +
			          IR::InstructionToString(inst);
			}
			return true;
		}
	}
...
EmbeddedFetchData DetectEmbeddedVertexFetch(const Decoder::Program&      decoded,
	...
	if (input_info == nullptr || !input_info->fetch_embedded ||
	    !CanMapEmbeddedFetchRegisterPair(input_info->fetch_attrib_reg) ||
	    !CanMapEmbeddedFetchRegisterPair(input_info->fetch_buffer_reg)) {
		return data;
	}
	...
	const auto attrib_reg =
	    static_cast<int64_t>(input_info->fetch_attrib_reg) +
	    static_cast<int64_t>(kEmbeddedFetchSgprShift);
	const auto buffer_reg =
	    static_cast<int64_t>(input_info->fetch_buffer_reg) +
	    static_cast<int64_t>(kEmbeddedFetchSgprShift);
	...
	const auto sgpr_count = static_cast<int64_t>(sgprs.size());
	if (attrib_reg >= 0 && attrib_reg < sgpr_count) {
		sgprs[static_cast<size_t>(attrib_reg)].type = EmbeddedFetchValueType::AttribTable;
	}
	if (attrib_reg + 1 >= 0 && attrib_reg + 1 < sgpr_count) {
		sgprs[static_cast<size_t>(attrib_reg + 1)].type = EmbeddedFetchValueType::AttribTable;
	}
	if (buffer_reg >= 0 && buffer_reg < sgpr_count) {
		sgprs[static_cast<size_t>(buffer_reg)].type = EmbeddedFetchValueType::BufferTable;
	}
	if (buffer_reg + 1 >= 0 && buffer_reg + 1 < sgpr_count) {
		sgprs[static_cast<size_t>(buffer_reg + 1)].type = EmbeddedFetchValueType::BufferTable;
	}
...
uint32_t RewriteEmbeddedVertexFetches(IR::Program& ir, ShaderVertexInputInfo* input_info,
	...
	for (auto& block: ir.blocks) {
		for (auto& inst: block.instructions) {
			...
			input_info->resource_fetch_components[resource_id] =
			    std::max(input_info->resource_fetch_components[resource_id],
...
bool TryRecompileImpl(std::span<const uint32_t> code, const CompileOptions& options,
	...
	if (!ValidateStageMetadata(options, error)) {
		return false;
	}

	ShaderVertexInputInfo  vertex_input_copy {};
	ShaderVertexInputInfo* vertex_input_info = nullptr;
	if (options.vertex_input_info != nullptr) {
		vertex_input_copy = *options.vertex_input_info;
		vertex_input_info = &vertex_input_copy;
	}
	...
	if (options.stage == ShaderType::Vertex && vertex_input_info != nullptr &&
	    vertex_input_info->fetch_embedded) {
		embedded_fetch =
		    DetectEmbeddedVertexFetch(decoded, vertex_input_info, ir.user_data_base,
		                              ir.user_data_count, options.wave_size);
		(void)RewriteEmbeddedVertexFetches(ir, vertex_input_info, embedded_fetch.loads);
	}
	...
	if (options.resource_snapshot != nullptr) {
		...
		if (options.user_data == nullptr && options.user_data_count > zero_user_data.size()) {
			if (error != nullptr) {
				*error = "synthetic user data exceeds fallback range";
			}
			return false;
		}
		...
		runtime.shader_base = options.shader_base;
	...
	info_options.vertex = vertex_input_info != nullptr ? vertex_input_info : &default_vertex;
	info_options.pixel =
	...
	if (!Spirv::EmitProgram(ir, resources, vertex_input_info, options.pixel_input_info,
	                        options.compute_input_info, spirv, &emit_error)) {
		if (dispatcher_fallback && error != nullptr) {
			*error = "dispatcher fallback failed after " + dispatcher_reason + ": " + emit_error;
	...
	compiled.program   = std::move(ir);
	compiled.resources = std::move(resources);
	if (options.dump_ir) {
		compiled.decoded_dump = std::move(decoded_dump);
		compiled.ir_dump      = std::move(ir_dump);
	}
...
bool TryRecompile(std::span<const uint32_t> code, const CompileOptions& options,
                  CompileResult& result, std::string* error) {
	try {
		return TryRecompileImpl(code, options, result, error);
	} catch (const std::bad_alloc&) {
		if (error != nullptr) {
			*error = "shader recompiler allocation failed";
		}
		return false;
	} catch (const std::exception&) {
		if (error != nullptr) {
			*error = "shader recompiler internal failure";
		}
		return false;
	} catch (...) {
		if (error != nullptr) {
			*error = "shader recompiler failed";
		}
		return false;
	}
}
