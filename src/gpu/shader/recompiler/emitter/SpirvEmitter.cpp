...
// src/graphics/shader/recompiler/emitter/SpirvEmitter.cpp at
...
bool EmitProgramImpl(const IR::Program& program, const IR::ResourceSnapshot& resources,
...
bool EmitProgram(const IR::Program& program, const IR::ResourceSnapshot& resources,
                 const ShaderVertexInputInfo* vertex_input_info,
                 const ShaderPixelInputInfo* pixel_input_info,
                 const ShaderComputeInputInfo* compute_input_info,
                 std::vector<uint32_t>& spirv, std::string* error) {
	try {
		return EmitProgramImpl(program, resources, vertex_input_info, pixel_input_info,
		                       compute_input_info, spirv, error);
	} catch (const Emitter::EmitFailure& failure) {
		Emitter::SetError(error, failure.what());
		return false;
	} catch (const std::bad_alloc&) {
		Emitter::SetError(error, "SPIR-V emitter allocation failed");
		return false;
	} catch (const std::exception&) {
		Emitter::SetError(error, "SPIR-V emitter internal failure");
		return false;
	} catch (...) {
		Emitter::SetError(error, "SPIR-V emitter failed");
		return false;
	}
}
