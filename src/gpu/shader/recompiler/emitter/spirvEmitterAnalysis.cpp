...
// src/graphics/shader/recompiler/emitter/spirvEmitterAnalysis.cpp at
...
namespace kajps5::gpu::shader::recompiler::Spirv::Emitter {
...
uint32_t PixelParameterLocation(const EmitterState& state, uint32_t attr) {
	...
	for (const auto& input: state.inputs) {
		...
		if (location < std::size(used_locations) && used_locations[location]) {
			...
			if (fallback_location >= std::size(used_locations)) {
				throw EmitFailure("pixel parameter locations exceed the supported range");
			}
...
[[noreturn]] void ExitDescriptorBindingFailure(const EmitterState& state,
	...
	(void)kind;
	(void)resource;
	throw EmitFailure(std::string("SPIR-V descriptor binding resolution failed: ") +
	                  (reason != nullptr ? reason : "invalid descriptor"));
}

DescriptorResourceBinding ResourceForDescriptor(EmitterState& state,
                                                IR::DescriptorBindingKind kind, uint32_t resource) {
