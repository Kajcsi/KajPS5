...
// src/graphics/shader/recompiler/decompiler/VectorAluOps.cpp at
...
bool DecodeVop1Sdwa(uint32_t pc, std::span<const uint32_t> code, uint32_t word_index,
	...
	if (word_index + 1u >= code.size()) {
		if (error != nullptr) {
			*error = std::format("truncated VOP1 SDWA instruction at pc 0x{:08x}", pc);
		}
		return false;
	}
...
bool DecodeVop1Dpp(uint32_t pc, std::span<const uint32_t> code, uint32_t word_index,
	...
	if (word_index + 1u >= code.size()) {
		if (error != nullptr) {
			*error = std::format("truncated VOP1 DPP instruction at pc 0x{:08x}", pc);
		}
		return false;
	}
...
bool DecodeVop2Sdwa(uint32_t pc, std::span<const uint32_t> code, uint32_t word_index,
	...
	if (word_index + 1u >= code.size()) {
		if (error != nullptr) {
			*error = std::format("truncated VOP2 SDWA instruction at pc 0x{:08x}", pc);
		}
		return false;
	}
...
bool DecodeVop2Dpp(uint32_t pc, std::span<const uint32_t> code, uint32_t word_index,
	...
	if (word_index + 1u >= code.size()) {
		if (error != nullptr) {
			*error = std::format("truncated VOP2 DPP instruction at pc 0x{:08x}", pc);
		}
		return false;
	}
...
bool DecodeVopcSdwa(uint32_t pc, std::span<const uint32_t> code, uint32_t word_index,
	...
	if (word_index + 1u >= code.size()) {
		if (error != nullptr) {
			*error = std::format("truncated VOPC SDWA instruction at pc 0x{:08x}", pc);
		}
		return false;
	}
...
bool DecodeVopcDpp(uint32_t pc, std::span<const uint32_t> code, uint32_t word_index,
	...
	if (word_index + 1u >= code.size()) {
		if (error != nullptr) {
			*error = std::format("truncated VOPC DPP instruction at pc 0x{:08x}", pc);
		}
		return false;
	}
...
bool DecodeVop3(uint32_t pc, std::span<const uint32_t> code, uint32_t word_index, Instruction& inst,
	...
	if (word_index + 1u >= code.size()) {
		if (error != nullptr) {
			*error = std::format("truncated VOP3 instruction at pc 0x{:08x}", pc);
		}
		return false;
	}
...
bool DecodeVop3p(uint32_t pc, std::span<const uint32_t> code, uint32_t word_index,
	...
	if (word_index + 1u >= code.size()) {
		if (error != nullptr) {
			*error = std::format("truncated VOP3P instruction at pc 0x{:08x}", pc);
		}
		return false;
	}
...
bool DecodeVintrp(uint32_t pc, std::span<const uint32_t> code, uint32_t word_index,
	...
	if (word_index >= code.size()) {
		if (error != nullptr) {
			*error = std::format("truncated VINTRP instruction at pc 0x{:08x}", pc);
		}
		return false;
	}
