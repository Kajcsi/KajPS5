...
// src/graphics/shader/recompiler/decompiler/MemoryOps.cpp at
...
bool DecodeSmem(uint32_t pc, std::span<const uint32_t> code, uint32_t word_index, Instruction& inst,
	...
	if (word_index + 1u >= code.size()) {
		if (error != nullptr) {
			*error = std::format("truncated SMEM instruction at pc 0x{:08x}", pc);
		}
		return false;
	}
...
bool DecodeMubuf(uint32_t pc, std::span<const uint32_t> code, uint32_t word_index,
	...
	if (word_index + 1u >= code.size()) {
		if (error != nullptr) {
			*error = std::format("truncated MUBUF instruction at pc 0x{:08x}", pc);
		}
		return false;
	}
...
bool DecodeMtbuf(uint32_t pc, std::span<const uint32_t> code, uint32_t word_index,
	...
	if (word_index + 1u >= code.size()) {
		if (error != nullptr) {
			*error = std::format("truncated MTBUF instruction at pc 0x{:08x}", pc);
		}
		return false;
	}
...
bool DecodeFlat(uint32_t pc, std::span<const uint32_t> code, uint32_t word_index, Instruction& inst,
	...
	if (word_index + 1u >= code.size()) {
		if (error != nullptr) {
			*error = std::format("truncated FLAT instruction at pc 0x{:08x}", pc);
		}
		return false;
	}
...
bool DecodeDs(uint32_t pc, std::span<const uint32_t> code, uint32_t word_index, Instruction& inst,
	...
	if (word_index + 1u >= code.size()) {
		if (error != nullptr) {
			*error = std::format("truncated DS instruction at pc 0x{:08x}", pc);
		}
		return false;
	}
