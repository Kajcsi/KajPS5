...
#include "gpu/shader/recompiler/decompiler/ExportOps.h"
#include "gpu/shader/recompiler/decompiler/ImageOps.h"
#include "gpu/shader/recompiler/decompiler/MemoryOps.h"
#include "gpu/shader/recompiler/decompiler/ScalarAluOps.h"
#include "gpu/shader/recompiler/decompiler/VectorAluOps.h"
...
std::string RawWordsToString(const Instruction& inst) {
	...
	for (uint32_t i = 0; i < inst.raw_count; i++) {
		...
		text += std::format("0x{:08x}", inst.raw[i]);
	}
	return text;
}

std::string FormatBinary(const Instruction& inst) {
	std::string text = std::format("0x{:08x}: {} {}", inst.pc, OpcodeToString(inst.opcode).c_str(),
...
std::string FormatMemory(const Instruction& inst) {
	...
	text += std::format(" ; offset={} offset2={} dwords={} bits={} dfmt={} nfmt={} signed={} "
...
std::string WithUnsupportedReason(const Instruction& inst, const std::string& text) {
	...
	return text + std::format(" ; family={} opcode=0x{:02x} raw=[{}] reason={}",
...
std::string FormatMimg(const Instruction& inst) {
	...
	switch (inst.opcode) {
		...
		case Opcode::ImageGather4H:
			text += std::format(" sample_flags={} addr_components={}",
	...
	if (inst.image_nsa_dwords != 0) {
		text += std::format(" nsa_dwords={} nsa_addr=", inst.image_nsa_dwords);
		const auto count =
		...
		for (uint32_t i = 0; i < count; i++) {
			...
			text += std::format("{}", inst.image_nsa_addr[i]);
		}
	}
	return text;
}
...
std::string FormatExp(const Instruction& inst) {
	std::string text = std::format("0x{:08x}: exp target=0x{:02x} en=0x{:x} done={} compr={} vm={}",
...
bool DecodeScalarSource(uint32_t code, uint32_t pc, Operand& operand, std::string* error) {
	...
	switch (code) {
		...
		default:
			if (error != nullptr) {
				*error = std::format("unsupported scalar source operand 0x{:08x} at pc 0x{:08x}",
				                     code, pc);
			}
			return false;
	}
...
bool ReadLiteralOperands(std::span<const uint32_t> code, uint32_t word_index, Instruction& inst,
	...
	if (word_index + inst.word_count >= code.size()) {
		if (error != nullptr) {
			*error = std::format("missing literal constant at pc 0x{:08x}", inst.pc);
		}
		return false;
	}
...
std::string OperandToString(const Operand& operand) {
	std::string text;
	switch (operand.kind) {
		case OperandKind::LiteralConstant: text = std::format("0x{:08x}", operand.value); break;
		case OperandKind::IntegerInlineConstant:
			text = std::format("{}", operand.signed_val);
			break;
		case OperandKind::FloatInlineConstant: text = std::format("{:f}", operand.float_val); break;
		case OperandKind::Sgpr: text = std::format("s{}", operand.reg); break;
		case OperandKind::Vgpr: text = std::format("v{}", operand.reg); break;
	...
	if (operand.sdwa_sel != 6 || operand.sdwa_sext) {
		text += std::format(".sdwa(sel={},sext={})", operand.sdwa_sel, operand.sdwa_sext ? 1u : 0u);
	}
	if (operand.absolute) {
		text += ".abs";
	}
	...
	if (operand.op_sel || operand.op_sel_hi || operand.negate_hi) {
		text += std::format(".opsel(lo={},hi={},neghi={})", operand.op_sel ? 1u : 0u,
		                    operand.op_sel_hi ? 1u : 0u, operand.negate_hi ? 1u : 0u);
	}
	if (operand.omod != 0) {
		text += std::format(".omod({})", operand.omod);
	}
	if (operand.clamp) {
		text += ".clamp";
	}
	if (operand.dpp) {
		text += std::format(".dpp(ctrl=0x{:x},fi={},bc={})", operand.dpp_ctrl,
...
std::string InstructionToString(const Instruction& inst) {
	if (inst.opcode == Opcode::Unsupported) {
		return std::format("0x{:08x}: unsupported family={} opcode=0x{:02x} raw=[{}] reason={}",
	...
	switch (inst.opcode) {
		case Opcode::SMovB32:
		case Opcode::SMovB64:
			return WithUnsupportedReason(inst, std::format("0x{:08x}: {} {}, {}", inst.pc,
		...
		case Opcode::SGetpcB64:
			return WithUnsupportedReason(inst, std::format("0x{:08x}: s_getpc_b64 {}", inst.pc,
			                                               OperandToString(inst.dst).c_str()));
		case Opcode::SSetpcB64:
			return WithUnsupportedReason(inst, std::format("0x{:08x}: s_setpc_b64 {}", inst.pc,
			                                               OperandToString(inst.src0).c_str()));
		case Opcode::SSetregB32:
			return WithUnsupportedReason(inst, std::format("0x{:08x}: s_setreg_b32 {}, {}", inst.pc,
			                                               OperandToString(inst.src0).c_str(),
		...
		case Opcode::SBarrier:
			return WithUnsupportedReason(inst, std::format("0x{:08x}: s_barrier", inst.pc));
		case Opcode::VNop:
			return WithUnsupportedReason(inst, std::format("0x{:08x}: v_nop", inst.pc));
		case Opcode::SEndpgm:
			return WithUnsupportedReason(inst, std::format("0x{:08x}: s_endpgm", inst.pc));
		case Opcode::SBranch:
		case Opcode::SCbranchScc0:
		case Opcode::SCbranchScc1:
		case Opcode::SCbranchVccz:
		...
		case Opcode::SCbranchExecnz:
			return WithUnsupportedReason(inst, std::format("0x{:08x}: {} 0x{:08x}", inst.pc,
			                                               OpcodeToString(inst.opcode).c_str(),
			                                               inst.branch_target));
