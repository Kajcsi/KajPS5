...
// src/graphics/shader/recompiler/ir/ShaderIRLog.cpp at
...
#include "gpu/shader/recompiler/ir/ShaderIRInternal.h"
...
std::string RegisterToString(Register reg) {
	switch (reg.file) {
		case RegisterFile::Scalar: return std::format("s{}", reg.index);
		case RegisterFile::Vector: return std::format("v{}", reg.index);
		...
		case RegisterFile::Scc:
			...
			return std::format("scc_like_{}", reg.index);
...
std::string OperandToString(const Operand& operand) {
	...
	switch (operand.kind) {
		...
		case OperandKind::ImmediateU32: text = std::format("0x{:08x}", operand.imm); break;
		case OperandKind::PcRelativeU32: text = std::format("pc_rel(0x{:08x})", operand.imm); break;
...
std::string MemoryInfoToString(const MemoryInfo& mem) {
	...
	if (mem.image_nsa_dwords != 0) {
		text += std::format(" image_nsa_dwords={} image_nsa_addr=", mem.image_nsa_dwords);
		const auto count =
		...
		for (uint32_t i = 0; i < count; i++) {
			...
			text += std::format("{}", mem.image_nsa_addr[i]);
		}
	}
	return text;
}
...
std::string ExportInfoToString(const ExportInfo& exp) {
	return std::format(" ; target={} index={} raw=0x{:02x} en=0x{:x} done={} compr={} vm={}",
...
std::string InputInfoToString(Opcode op, const InputInfo& input) {
	...
	return std::format(" ; input_attr={} input_chan={}", input.attr, input.chan);
}
...
std::string InstructionToString(const Instruction& inst) {
	std::string text = std::format("0x{:08x}: ", inst.pc);
...
std::string ProgramToString(const Program& program) {
	std::string text;
	text += std::format("mode={} masks={} fallback={} reason={}\n",
	...
	for (const auto& block: program.blocks) {
		text += std::format("block_{} pc=0x{:08x} end=0x{:08x} preds={} succs={} {}\n", block.id,
