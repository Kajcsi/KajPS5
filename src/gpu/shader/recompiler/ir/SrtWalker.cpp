...
// src/graphics/shader/recompiler/ir/SrtWalker.cpp at
...
std::string Diagnostic(const Program& program, uint32_t pc, const std::string& message) {
	return std::format("shader SRT: hash=0x{:016x} stage={} pc=0x{:08x} {}", program.shader_hash,
...
private:
	...
	bool CollectDescriptor(uint32_t source, uint32_t use_pc, std::string* error) {
		...
		if (descriptor == nullptr) {
			...
			return Fail(use_pc, error, std::format("invalid descriptor source {}", source));
		}
	...
	bool CollectValue(uint32_t id, uint32_t use_pc, std::string* error) {
		if (id >= m_program.provenance.values.size()) {
			return Fail(use_pc, error, std::format("invalid scalar value {}", id));
		}
		if (m_state[id] == 2) {
			return true;
		}
		if (m_state[id] == 1) {
			return Fail(use_pc, error, std::format("cyclic scalar value {}", id));
		}
		m_state[id]       = 1;
		const auto& value = m_program.provenance.values[id];
		if (value.op == ScalarValueOp::Phi) {
...
public:
	...
	bool Evaluate(uint32_t id, uint32_t& result, std::string* error) {
		if (id >= m_program.provenance.values.size()) {
			return Fail(error, std::format("invalid scalar value {}", id));
		}
		if (m_state[id] == 2) {
			result = m_values[id];
			return true;
		...
		if (m_state[id] == 1) {
			return Fail(error, std::format("cyclic scalar value {}", id));
		}
		m_state[id]       = 1;
		const auto& value = m_program.provenance.values[id];
		uint32_t    out   = 0;
		switch (value.op) {
			case ScalarValueOp::UserData:
				if (value.imm < m_program.user_data_base ||
					...
					return Fail(error, std::format("user SGPR {} is unavailable", value.imm));
				}
			...
			case ScalarValueOp::Phi: {
				...
				if (first == value.phi_args.end()) {
					return Fail(error, std::format("empty scalar phi {}", id));
				}
				...
				for (const auto arg: value.phi_args) {
					...
					if (other != out) {
						return Fail(error,
						            std::format("scalar phi {} has runtime-dependent values", id));
					}
				}
				break;
			}
			...
			case ScalarValueOp::Unknown:
				return Fail(error, std::format("scalar value {} is unresolved", id));
			default:
...
private:
	...
	bool EvaluateOperation(const ScalarValue& value, uint32_t& result, std::string* error) {
		const auto count = ScalarValueArgCount(value.op);
		if (count == 0 || count > 3) {
			return Fail(error, std::format("unsupported scalar operation {}",
		...
		return ApplyOperation(value.op, args, result) ||
		       Fail(error, std::format("unsupported scalar operation {}",
		                               static_cast<uint32_t>(value.op)));
	}
	...
	bool Read(const ScalarValue& value, uint32_t& result, std::string* error) {
		...
		if (buffer) {
			...
			if (immediate < 0) {
				return Fail(error,
				            std::format("ReadConstBuffer pc=0x{:08x} has negative immediate {}",
				                        value.pc, immediate));
			}
			...
			if (aligned_offset > size || size - aligned_offset < sizeof(uint32_t)) {
				return Fail(error,
				            std::format("ReadConstBuffer pc=0x{:08x} offset={} exceeds size={}",
			...
			if (!AddSignedAddress(base_aligned, relative, address)) {
				return Fail(error,
				            std::format("ReadConst pc=0x{:08x} address base=0x{:016x} offset={} "
				                        "is outside the 48-bit address space",
				                        value.pc, base_aligned, relative));
			}
		}
		if (m_runtime.read_memory == nullptr) {
			return Fail(error, std::format("{} pc=0x{:08x} requires an explicit checked memory reader",
			                               buffer ? "ReadConstBuffer" : "ReadConst", value.pc));
		}
		if (!m_runtime.read_memory(m_runtime.userdata, address, &result)) {
			return Fail(
			    error, std::format("ReadConst pc=0x{:08x} failed at 0x{:016x}", value.pc, address));
		}
		return true;
	}
		...
		if (descriptor == nullptr || dynamic ||
			...
			if (error != nullptr) {
				*error = Diagnostic(program, request.use_pc,
				                    std::format("descriptor source {} is {}", request.source,
