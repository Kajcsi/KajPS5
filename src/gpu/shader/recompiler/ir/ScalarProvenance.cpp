...
// src/graphics/shader/recompiler/ir/ScalarProvenance.cpp at
		...
		for (const auto& block: m_program.blocks) {
			for (const auto predecessor: block.predecessors) {
				if (predecessor >= block_count) {
					return Fail(error, std::format("scalar provenance has invalid predecessor {}",
					                               predecessor));
				}
			}
			for (const auto successor: block.successors) {
				if (successor >= block_count) {
					return Fail(error, std::format("scalar provenance has invalid successor {}",
					                               successor));
				}
			}
	...
	bool ValidateScalarMemoryGroups(const BasicBlock& block, std::string* error) const {
		for (size_t index = 0; index < block.instructions.size();) {
			...
			if (count == 0 || first.memory.component_index != 0 ||
				...
				return Fail(error,
				            std::format("malformed scalar-memory group at pc=0x{:x}", first.pc));
			}
			for (uint32_t component = 0; component < count; component++) {
				...
				if (inst.op != first.op || inst.pc != first.pc ||
					...
					return Fail(error,
					            std::format("malformed scalar-memory component {} at pc=0x{:x}",
					                        component, first.pc));
				}
			}
			index += count;
...
std::string ScalarValueToString(const ScalarProvenance& provenance, uint32_t value) {
	...
	switch (node.op) {
		...
		case ScalarValueOp::UserData: return std::format("ud{}", node.imm);
		case ScalarValueOp::Constant: return std::format("0x{:08x}", node.imm);
		case ScalarValueOp::PcRelativeLow: return std::format("pc_lo+0x{:x}", node.imm);
		case ScalarValueOp::PcRelativeHigh: return std::format("pc_hi+0x{:x}", node.imm);
		case ScalarValueOp::ReadConst:
			return std::format("read_const({}, {}, {}, +{})", node.args[0], node.args[1],
			                   node.args[2], node.imm);
		case ScalarValueOp::ReadConstBuffer:
			return std::format("read_const_buffer({}, {}, {}, {}, {}, +{})", node.args[0],
			                   node.args[1], node.args[2], node.args[3], node.args[4], node.imm);
		case ScalarValueOp::Phi: return std::format("phi{}", value);
		case ScalarValueOp::BitFieldMaskU32:
			return std::format("bfm_u32({}, {})", node.args[0], node.args[1]);
		default: return std::format("value{}", value);
	}
}
