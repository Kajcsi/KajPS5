...
// src/graphics/shader/recompiler/ir/SrtPatcher.cpp at
...
bool PatchSrtReads(Program& program, std::string* error) {
	...
	for (const auto& read: program.srt.reads) {
		if (read.value >= program.provenance.values.size() ||
			...
			if (error != nullptr) {
				*error = std::format("invalid SRT read value={} flat_offset={}", read.value,
				                     read.flat_offset);
			}
			return false;
		}
		...
		if (!inserted && it->second != read.flat_offset) {
			if (error != nullptr) {
				*error = std::format("SRT read value {} has conflicting flat offsets", read.value);
			}
			return false;
		}
		...
		if (!unique && value->second != read.value) {
			if (error != nullptr) {
				*error = std::format("SRT flat offset {} has conflicting values", read.flat_offset);
			}
			return false;
		}
	}
	...
	for (const auto value: program.srt.dynamic_reads) {
		if (value >= program.provenance.values.size() || offsets.contains(value) ||
			...
			if (error != nullptr) {
				*error = std::format("invalid dynamic SRT read value={}", value);
			}
			return false;
		}
	}
	...
	for (const auto source: program.srt.dynamic_sources) {
		if (source == ScalarProvenance::Undefined ||
			...
			if (error != nullptr) {
				*error = std::format("invalid dynamic descriptor source={}", source);
			}
			return false;
		}
	}
		...
		for (auto& inst: block.instructions) {
			...
			if (const auto found = offsets.find(inst.scalar_value); found != offsets.end()) {
				...
				if ((inst.op == Opcode::SLoadDword) != (value_op == ScalarValueOp::ReadConst)) {
					if (error != nullptr) {
						*error = std::format("SRT read value {} has the wrong scalar-load producer",
						                     found->first);
					}
					return false;
				}
	...
	for (const auto& [value, found]: matched) {
		if (!found) {
			if (error != nullptr) {
				*error = std::format("SRT read value {} has no scalar-load producer", value);
			}
			return false;
		}
	}
