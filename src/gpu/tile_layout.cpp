...
static uint32_t SetLinearMipChainLayout(uint32_t levels, const uint32_t* mip_pitch,
	...
	for (int32_t l = static_cast<int32_t>(levels) - 1; l >= 0; l--) {
		...
		if (offset > UINT32_MAX || mip_size[level] > UINT32_MAX - offset) return 0;
	...
	const uint64_t aligned = (offset + 255u) & ~uint64_t {255u};
	return aligned <= UINT32_MAX ? static_cast<uint32_t>(aligned) : 0;
}
...
static bool SetMicroMipLayout(const TextureBlockLayout& block, uint32_t width, uint32_t height,
	...
	for (int32_t l = static_cast<int32_t>(levels) - 1; l >= 0; --l) {
		...
		const uint64_t size = static_cast<uint64_t>(padded_width) * padded_height * block.bytes;
		if (size > UINT32_MAX || offset > UINT32_MAX - size) return false;
		if (level_sizes != nullptr) level_sizes[level] = {static_cast<uint32_t>(size), static_cast<uint32_t>(offset)};
		if (padded_size != nullptr) {
	...
	if (total_size != nullptr) *total_size = {static_cast<uint32_t>(offset), block.block_size};
	return true;
}

static bool SetMacroMipLayout(const TextureBlockLayout& block, uint32_t tile, uint32_t width,
	...
	uint64_t offset = first_tail < levels ? block.block_size : 0;
	for (int32_t l = static_cast<int32_t>(first_tail) - 1; l >= 0; --l) {
		...
		const uint64_t size = static_cast<uint64_t>(padded_width) * padded_height * block.bytes;
		if (size > UINT32_MAX || offset > UINT32_MAX - size) return false;
		if (level_sizes != nullptr) level_sizes[level] = {static_cast<uint32_t>(size), static_cast<uint32_t>(offset), static_cast<uint32_t>(size), static_cast<uint32_t>(offset)};
		if (padded_size != nullptr) {
			padded_size[level] = {padded_width * block.texel_width,
			                      padded_height * block.texel_height};
		}
	...
	for (uint32_t level = first_tail; level < levels; ++level) {
		...
		const uint64_t tail_size = static_cast<uint64_t>(mip_width) * mip_height * block.bytes;
		if (tail_size > UINT32_MAX || linear_offset > UINT32_MAX - tail_size) return false;
		if (level_sizes != nullptr) {
			level_sizes[level] = {static_cast<uint32_t>(tail_size),
			                      static_cast<uint32_t>(linear_offset),
			                      block.block_size,
			                      0,
			                      tail.x,
			                      tail.y};
...
static bool TileGetTextureVolumeLayoutImpl(uint32_t format, uint32_t width, uint32_t height,
                                           uint32_t depth, uint32_t levels, uint32_t tile,
                                           TileVolumeLayout& out) {
...
bool TileGetTextureVolumeLayout(uint32_t format, uint32_t width, uint32_t height,
                                uint32_t depth, uint32_t levels, uint32_t tile,
                                TileVolumeLayout& layout) {
	if (width > 16384 || height > 16384 || depth > 16384) return false;
	TileVolumeLayout calculated {};
	if (!TileGetTextureVolumeLayoutImpl(format, width, height, depth, levels, tile,
	                                    calculated)) {
		return false;
	}
	layout = calculated;
...
bool TileGetBlockXor(const TileBlockLayout& layout, uint32_t block_x, uint32_t block_y,
	...
	if (layout.family == TileBlockFamily::Depth64KB && layout.bytes_per_element == 8) {
		...
		calculated_offset = Depth64KB64XOffsetBytes(block_x * layout.block_width) ^
		...
		switch (layout.bytes_per_element) {
			case 1: calculated_offset = Gen5RenderTargetOffsetInBlock<uint8_t>(x, y); break;
			case 2: calculated_offset = Gen5RenderTargetOffsetInBlock<uint16_t>(x, y); break;
			case 4: calculated_offset = Gen5RenderTargetOffsetInBlock<uint32_t>(x, y); break;
			case 8: calculated_offset = Gen5RenderTargetOffsetInBlock<uint64_t>(x, y); break;
			case 16: calculated_offset = Gen5RenderTargetOffsetInBlock<Uint128>(x, y); break;
			default: return false;
		}
	} else {
		calculated_offset = 0;
	...
	if (layout.family == TileBlockFamily::RenderTarget64KB ||
		...
		calculated_offset ^= ((block_z & 8u) << 5u) ^ ((block_z & 4u) << 7u) ^ ((block_z & 2u) << 9u) ^
		               ((block_z & 1u) << 11u);
	}
	if (calculated_offset >= layout.block_size ||
	    calculated_offset % layout.bytes_per_element != 0) return false;
	byte_offset = calculated_offset;
	return true;
}
...
bool TileGetHtileSize(uint32_t width, uint32_t height, TileSizeAlign& htile_size) {
	...
	const TileSizeAlign result {static_cast<uint32_t>(size), 32768};
	htile_size = result;
	return true;
}
...
bool TileGetDepthSize(uint32_t width, uint32_t height, uint32_t pitch, uint32_t z_format,
	...
	if (width > 0 && width <= 16384 && height > 0 && height <= 16384 &&
		...
		if (depth_bytes_total <= UINT32_MAX && stencil_bytes_total <= UINT32_MAX && htile_valid) {
			const TileSizeAlign calculated_depth {static_cast<uint32_t>(depth_bytes_total), 65536};
			const TileSizeAlign calculated_stencil = stencil_format == 1
			                   ? TileSizeAlign {static_cast<uint32_t>(stencil_bytes_total), 65536}
			                   : TileSizeAlign {};
			depth_size = calculated_depth;
			stencil_size = calculated_stencil;
...
bool TileGetRenderTargetPitch(uint32_t width, uint32_t bytes_per_element,
                              uint32_t& pitch, uint32_t num_fragments_log2) {
	...
	const uint64_t calculated_pitch = (static_cast<uint64_t>(width) + block_width - 1u) &
	                       ~static_cast<uint64_t>(block_width - 1u);
	if (calculated_pitch > UINT32_MAX) return false;
	pitch = static_cast<uint32_t>(calculated_pitch);
	return true;
}

bool TileGetDepthPitch(uint32_t width, uint32_t bytes_per_element,
                       uint32_t& pitch, uint32_t num_fragments_log2) {
	return TileGetRenderTargetPitch(width, bytes_per_element, pitch, num_fragments_log2);
}
...
bool TileGetRenderTargetSize(uint32_t width, uint32_t height, uint32_t pitch,
	...
	uint32_t block_width  = 0;
	uint32_t block_height = 0;
	uint32_t expected_pitch = 0;
	if (height == 0 || pitch == 0 ||
	    !Gen5Msaa64KBBlockSizeFromElementBytes(bytes_per_element, num_fragments_log2, &block_width,
	                                           &block_height) ||
	    !TileGetRenderTargetPitch(width, bytes_per_element, expected_pitch,
	                              num_fragments_log2) || pitch != expected_pitch) {
		return false;
	}
	...
	const uint64_t size = static_cast<uint64_t>(pitch) * padded_height * bytes_per_element *
	                      (uint64_t {1} << num_fragments_log2);
	if (size == 0 || size > UINT32_MAX) {
		return false;
	}
	const TileSizeAlign result {static_cast<uint32_t>(size), 65536};
	total_size = result;
	return true;
}
...
bool TileGetRenderTargetMipLayout(uint32_t width, uint32_t height, uint32_t pitch,
	...
	TileSizeAlign calculated_total {};
	std::array<TileSizeOffset, 16> calculated_levels {};
	std::array<TilePaddedSize, 16> calculated_padded {};
	if (!TileGetTextureSize(format, width, height, pitch, levels,
	                        Prospero::GpuEnumValue(Prospero::TileMode::kRenderTarget),
	                        &calculated_total, calculated_levels.data(),
	                        calculated_padded.data()) || calculated_total.size == 0 ||
	    calculated_total.align != 65536) {
		return false;
	}
	total_size = calculated_total;
	if (level_sizes != nullptr) std::copy_n(calculated_levels.begin(), levels, level_sizes);
	if (padded_size != nullptr) std::copy_n(calculated_padded.begin(), levels, padded_size);
	return true;
}

bool TileGetTextureSize(uint32_t format, uint32_t width, uint32_t height, uint32_t pitch,
                        uint32_t levels, uint32_t tile, TileSizeAlign* total_size,
                        TileSizeOffset* level_sizes, TilePaddedSize* padded_size) {
	// Kyty's layout calculation derives pitch from dimensions and tiling.
	(void)pitch;
	if (total_size == nullptr || width == 0 || width > 16384 || height == 0 ||
	    height > 16384 || levels == 0 || levels > 16) {
		return false;
	}
	const auto element_bytes = Prospero::NumBytesPerElement(format);
	if (element_bytes != 0 &&
	    static_cast<uint64_t>(width) * height * element_bytes > UINT32_MAX) {
		return false;
	}
	const auto block_bytes = Prospero::BlockCompressedBytesPerBlock(format);
	if (block_bytes != 0 &&
	    static_cast<uint64_t>((width + 3u) / 4u) * ((height + 3u) / 4u) *
	            block_bytes >
	        UINT32_MAX) {
		return false;
	}
	...
	if (const uint32_t bytes_per_element = Prospero::NumBytesPerElement(format);
		...
		for (uint32_t l = 0; l < levels; l++) {
			...
			if (mip_pitch[l] == 0 || mip_size[l] == 0) return false;
		}

		const uint32_t total = SetLinearMipChainLayout(levels, mip_pitch, mip_height, mip_size,
		                                               calculated_levels.data(), calculated_padded.data());
		if (total == 0) return false;
		calculated_total = {total, 256};
		*total_size = calculated_total;
		if (level_sizes != nullptr) std::copy_n(calculated_levels.begin(), levels, level_sizes);
		if (padded_size != nullptr) std::copy_n(calculated_padded.begin(), levels, padded_size);
		return true;
	...
	if (GetTextureBlockLayout(format, tile, block)) {
		if (tile == 1) {
			if (!SetMicroMipLayout(block, width, height, levels, &calculated_total,
			                       calculated_levels.data(), calculated_padded.data())) return false;
		} else {
			if (!SetMacroMipLayout(block, tile, width, height, levels, &calculated_total,
			                       calculated_levels.data(), calculated_padded.data())) return false;
		}
		if (calculated_total.size == 0) return false;
		*total_size = calculated_total;
		if (level_sizes != nullptr) std::copy_n(calculated_levels.begin(), levels, level_sizes);
		if (padded_size != nullptr) std::copy_n(calculated_padded.begin(), levels, padded_size);
		return true;
	...
	if (auto bytes_per_block = Prospero::BlockCompressedBytesPerBlock(format);
		...
		for (uint32_t l = 0; l < levels; l++) {
			...
			if (aligned_blocks_w == 0 || mip_size[l] == 0) return false;
...
bool TileGetTextureTotalSize(uint32_t format, uint32_t width, uint32_t height, uint32_t depth,
	...
	if (volume_texture) {
		...
		if (TileGetTextureVolumeLayout(format, width, height, depth, levels, tile, volume)) {
			if (volume.total_size > UINT32_MAX) return false;
			const TileSizeAlign result {static_cast<uint32_t>(volume.total_size),
			                            tile == 5 ? 4096u : 65536u};
			total_size = result;
			return true;
		}
	...
	if (!TileGetTextureSize(format, width, height, pitch, levels, tile, &slice_size,
	...
	slice_size.size = static_cast<uint32_t>(total);
	total_size = slice_size;
	return true;
}

bool TileGetTexturePitch(uint32_t format, uint32_t width, uint32_t levels, uint32_t tile,
                         uint32_t& pitch) {
	if (width == 0 || width > 16384 || levels == 0 || levels > 16) return false;
	if (tile != 0 && tile != 1 && tile != 5 && tile != 9 && tile != 17 &&
	    tile != 24 && tile != 27) return false;
	if (tile == 0 && Prospero::NumBytesPerElement(format) == 0 &&
	    Prospero::BlockCompressedBytesPerBlock(format) == 0) return false;
	if (tile != 0) {
		TextureBlockLayout block {};
		if (!GetTextureBlockLayout(format, tile, block)) return false;
	}
	uint32_t calculated_pitch = width;
	...
	if (tile == 27) {
		if (const uint32_t bytes_per_element = Prospero::NumBytesPerElement(format);
			...
			uint32_t block_width  = 0;
			uint32_t block_height = 0;
			if (!Gen5Thin64KBBlockSizeFromElementBytes(bytes_per_element, &block_width,
			                                           &block_height)) return false;
			calculated_pitch = AlignUp(calculated_pitch, block_width);
		}
	}
	...
	if (tile == 0) {
		if (const auto bytes_per_element = Prospero::NumBytesPerElement(format);
		    bytes_per_element != 0) {
			if (calculated_pitch > UINT32_MAX / bytes_per_element) return false;
			calculated_pitch = AlignUp(calculated_pitch * bytes_per_element, 256u) /
			                   bytes_per_element;
		}
	}
	uint32_t bytes_per_element       = 0;
	...
	if ((tile == 9 || tile == 17) &&
	    Gen5Standard64KBLayout(format, &bytes_per_element, &texels_per_element_wide,
	                           &texels_per_element_tall, &block_width_log2, &block_height_log2)) {
		calculated_pitch = AlignUp(calculated_pitch,
		                           (1u << block_width_log2) * texels_per_element_wide);
	}
	if (tile == 24) {
		const uint32_t depth_bytes_per_element = Prospero::NumBytesPerElement(format);
		uint32_t       block_width       = 0;
		uint32_t       block_height      = 0;
		if (depth_bytes_per_element <= 8 &&
		    Gen5Thin64KBBlockSizeFromElementBytes(depth_bytes_per_element, &block_width, &block_height)) {
			calculated_pitch = AlignUp(calculated_pitch, block_width);
		}
	}
